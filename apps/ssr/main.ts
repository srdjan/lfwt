...
import { timeNow, fetchJson, fetchTodo, bumpCounter } from "../../packages/core/src/services.ts"
...
import { hal, tmpl, prop, HREF, REL } from "../../packages/web/src/hal_forms.ts"
...
const Registry = defineRegistry<{ req: Request; url: URL; state?: any }>()({
  ...
  rate: rateLimit(20, 60_000, (r) => getApiKey(r) ?? "anon", async (key) => deps.kv.incr ? deps.kv.incr(String(key)) : (await bumpCounter({ kv: deps.kv })(String(key)))),
...
const routes = router([
  { method: "GET", path: "/", handler: async ({ url }) => {
    const html = `<!doctype html><html><head>
      <meta charset="utf-8" /><title>LFTS SSR</title>
      ...
      <h1>LFTS — Branded Types + HAL/HATEOAS + Macros</h1>
      <div class="card">
  ...
  { method: "GET", path: "/time", handler: async () => new Response(`<span id="t">${timeNow(deps)().toISOString()}</span>`, { headers: { "content-type": "text/html" } }) },
  { method: "POST", path: "/echo", handler: async (ctx) => {
  ...
  { method: "GET", path: "/ext", handler: async (ctx) => {
      const key = getApiKey(ctx.req); if (!key) return new Response("Unauthorized", { status: 401 })
      const todo = await resilientTodo(deps)(key, PositiveIntFrom(1))
      return new Response(`<code>${todo ? todo.title : "failed"}</code>`, { headers: { "content-type": "text/html" } })
  }},
  { method: "GET", path: "/.well-known/hal", handler: async ({ url }) => {
      const doc = hal({
      ...
      }, undefined, { name: "LFTS SSR Demo (Branded)" })
      return new Response(JSON.stringify(doc, null, 2), { headers: { "content-type": "application/hal+json" } })
  }},
  { method: "GET", path: "/forms", handler: async () => {
      const forms = hal({ [REL("self")]: { href: HREF("/forms") } }, {
        echo: tmpl({ title: "Echo", method: "POST", target: "/echo", properties: [prop({ name: "msg", prompt: "Message", required: true })] }),
  ...
  { method: "GET", path: "/admin", handler: async () => new Response("Welcome, admin!", { headers: { "content-type": "text/plain" } }) },
])

const base = routes({})

const withUserState = (req: Request) => ({ user: { id: "demo", roles: (req.headers.get("x-roles") ?? "").split(",").filter(Boolean) } })
